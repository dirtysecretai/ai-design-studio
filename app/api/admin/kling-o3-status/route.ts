...
// POST /api/admin/kling-o3-status
// Polls a FAL queue Kling O3 (Omni Image) job. On completion, re-hosts images on Vercel Blob and saves to DB.
export async function POST(req: Request) {
  ...
  try {
    ...
    const { falEndpoint, prompt, outputFormat, aspectRatio, quality, referenceImageUrls, queuedAt } = body
    ...
    if (status.status === 'COMPLETED') {
      ...
      try {
        ...
        if (!targetUserId) {
          console.error('kling-o3-status: no session user — skipping DB save')
        }
        if (targetUserId) {
        ...
        console.error('kling-o3-status: DB save failed (non-fatal):', dbErr)
      }

      await releaseQueueSlot(requestId, false)
      console.log(`✓ Kling O3 completed [${requestId}] ${hostedImages.length} image(s)`)
      return NextResponse.json({
        status: 'completed',
    ...
    console.error('kling-o3-status error:', error)
    if (error.body) console.error('kling-o3-status error body:', JSON.stringify(error.body))
