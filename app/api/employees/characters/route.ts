...
/**
 * Character projects, mirroring the films route.
 *
...
const UNTITLED = 'Untitled character'
const CHARACTER_DESIGN = BUILT_IN_EMPLOYEES.find(e => e.id === 'emp-character-design')!
...
// GET /api/employees/characters — the project list
export async function GET(): Promise<Response> {
  ...
  return jsonPrivate({ projects: films })
}

// POST /api/employees/characters — start a character project
export async function POST(req: NextRequest): Promise<Response> {
  ...
  const chat = await prisma.chat.create({
    data: {
      ...
      agentMode: 'accept',
      systemPrompt: CHARACTER_DESIGN.text,
      skills: CHARACTER_DESIGN.skills,
    },
    select: { id: true, title: true, updatedAt: true },
  })
  return jsonPrivate({
...
// PATCH /api/employees/characters — rename { id, title }
...
// DELETE /api/employees/characters?id=123 — close a project.
